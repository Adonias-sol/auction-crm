...
class FeeConfig(models.Model):
    ...
    Only one row should have isActive=True at a time — enforce that in the
    ...
    configuredBy = models.ForeignKey(settings.AUTH_USER_MODEL, on_delete=models.SET_NULL, null=True, blank=True)
    configuredAt = models.DateTimeField(auto_now=True)
    isActive = models.BooleanField(default=True)

    def __str__(self):
        return f"{self.percentage}% ({'active' if self.isActive else 'inactive'})"

    @classmethod
    def get_active_percentage(cls):
        active = cls.objects.filter(isActive=True).first()
...
class Winner(models.Model):
    ...
    winnerEmail = models.EmailField(blank=True)  # dropped null=True — see note above
